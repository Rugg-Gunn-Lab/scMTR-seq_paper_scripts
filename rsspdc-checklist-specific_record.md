...
- [x] Uses git (or other source control tool)
...
<ul>
	<li>
	Once you have selected your license include a plain text copy of it in the root of your repository in a file named LICENSE.
...
- [x] Record is Citable
	...
	- [x]  	🥈Silver *(easy)*: bronze plus the project has persistent resolvable identifier such as a DOI or SWHID, with which it can be referenced, which has been minted for the project using a tool like [zenodo](https://zenodo.org/) or [Software Heritage's Archive](https://www.softwareheritage.org/) to store an archival copy of the project.
...
- [x] Suitable automations are in place
	- [x]  	🥉Bronze *(easy)*: 1 from this list of processes are automated
		- [ ] use of an environment management tool
		- [x] use of a literate programming / computational notebook
...
- [x] The project has a suitable governance model
	- [x]  	🥉Bronze *(easy)*: The governance model is clearly communicated
	- [ ]  	🥈Silver *(easy)*: Bronze Plus - Project has continuity planning in place (2 or more from)
		...
		- [x] Plans of action in the event project admin(s) are no longer available
...
You may however still encounter people referring to and making use of parts of the work and asking questions about it.
Therefore basis policies for any community spaces and user support may still be sensible to indicate.
Continuity planning is also still important as others may need to pick up where you left off at some point in the future.
<ul>
	<li>
